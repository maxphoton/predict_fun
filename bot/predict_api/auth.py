...
async def get_jwt_token_predict_account(
    ...
    try:
        ...
        if message_response.status_code != 200:
            logger.error(f"Ошибка получения сообщения: {message_response.status_code} - {message_response.text}")
            return None
        ...
        if jwt_response.status_code != 200:
            logger.error(f"Ошибка получения JWT токена: {jwt_response.status_code} - {jwt_response.text}")
            return None
