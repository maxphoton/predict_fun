...
from database import get_order_by_hash, get_user, get_user_orders, update_order_status
...
async def get_orders_list_data(dialog_manager: DialogManager, **kwargs):
    ...
    has_active_orders = any(order.get("status") == "OPEN" for order in all_orders)
...
async def cancel_order_input_handler(
    ...
    if not order:
        await message.answer(
            f"❌ Order <code>{order_hash}</code> not found in database."
        )
        manager.dialog_data["cancel_mode"] = False
        return
    ...
    if order.get("telegram_id") != telegram_id:
        ...
        logger.warning(
            f"User {telegram_id} attempted to cancel another user's order {order_hash} (owner: {order.get('telegram_id')})"
        )
        manager.dialog_data["cancel_mode"] = False
        return
    ...
    try:
        ...
        if result.get("success", False):
            ...
            if order_api_id in removed:
                ...
                await message.answer(
                    f"✅ Order <code>{order_hash}</code> successfully cancelled."
                )
                ...
                if order_api_id in noop:
                    ...
                    await update_order_status(order_hash, "CANCELLED")
                    logger.info(
                        f"Order {order_hash} was already cancelled/filled for user {telegram_id}"
                    )
                else:
                    await message.answer(
                    ...
                    logger.warning(
                        f"Failed to cancel order {order_hash} for user {telegram_id}: order not in removed or noop"
                    )
        else:
            error_message = f"❌ Failed to cancel order <code>{order_hash}</code>."
            await message.answer(error_message)
            logger.warning(
                f"Failed to cancel order {order_hash} for user {telegram_id}: API returned success=False"
            )

    except Exception as e:
        await message.answer(
            f"❌ Error cancelling order <code>{order_hash}</code>: {str(e)}"
        )
        logger.error(
            f"Error cancelling order {order_hash} for user {telegram_id}: {e}",
            exc_info=True,
        )
