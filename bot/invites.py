        ...
        for _ in range(count):
            ...
            while True:
                ...
                async with conn.execute(
                    "SELECT id FROM invites WHERE invite = ?",
                    (invite_code,)
                ) as cursor:
            ...
            # Вставляем инвайт
            await conn.execute(
                "INSERT INTO invites (invite) VALUES (?)",
                (invite_code,)
            )
    ...
    async with aiosqlite.connect(DB_PATH) as conn:
        ...
        try:
            ...
            if row:
                ...
                await conn.commit()
                logger.info(f"Инвайт {invite_code} использован пользователем {telegram_id}")
                return True
            else:
                await conn.rollback()
                return False
...
async def get_invites_statistics() -> Dict:
    ...
    async with aiosqlite.connect(DB_PATH) as conn:
        ...
        # Использованных
        async with conn.execute("SELECT COUNT(*) FROM invites WHERE telegram_id IS NOT NULL") as cursor:
