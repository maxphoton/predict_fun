...
Please enter a valid 5-character code:"""
    ...
    logger.info(f"Пользователь {telegram_id} пытается установить реферальный код: {referral_code}")
...
The referral code has been set for your account."""
            ...
            logger.info(f"Пользователь {telegram_id} успешно установил реферальный код: {referral_code}")
        else:
            await message.answer(
...
Please check the code and try again, or contact support if the problem persists."""
            ...
            logger.warning(f"Не удалось установить реферальный код для пользователя {telegram_id}: {referral_code}")
    
    except Exception as e:
        logger.error(f"Ошибка при установке реферального кода для пользователя {telegram_id}: {e}", exc_info=True)
        await message.answer(
